...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join('toolkit_config.py','project1')
...
def get_tics(pth):
    ...
    for key, value in dic.items():
        dic1[key.lower().strip('"')] = value.lower().strip('"')
    return dic1
...
# ----------------------------------------------------------------------------
def read_dat(tic):
    data_path = os.path.join(DATDIR, f"{tic}_prc.dat")
    with open(data_path, 'rt') as dataopen:
        datalines = dataopen.read().splitlines()
    return datalines
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        ...
        value = line[length: length + width].strip()
        dic2[column] = value
        length = width+length
    return dic2
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        ...
        if len(tickers_lst) > 0:
            for lst in tickers_lst:
                if lst in tic_exchange_dic:
                    continue
                else:
                    raise Exception('not a key of tic_exchange_dic')
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        ...
        if len(col_lst) > 0:
            for lst in col_lst:
                if lst not in COLUMNS:
                    raise Exception('not found in COLUMNS')
# ----------------------------------------------------------------------------
#   Please complete the body of this function so it matches its docstring
#   description. See the assessment description file for more information.
# ----------------------------------------------------------------------------
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # <COMPLETE THIS PART>
    data_dic ={}
    for key in tickers_lst:

        data_dic[key] ={
            'exchange': tic_exchange_dic[key],
            'data':[]
        }
        datalines = read_dat(key)
        for line in datalines:
            data = line_to_dict(line)
            data_col = {col: data[col] for col in col_lst}
            data_dic[key]['data'].append(data_col)
        return data_dic
# ----------------------------------------------------------------------------
#   Please complete the body of this function so it matches its docstring
#   description. See the assessment description file for more information.
...
def create_json(data_dict, pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth,'r') as dataopen:
        json.dump(data_dict,dataopen,indent=4)
